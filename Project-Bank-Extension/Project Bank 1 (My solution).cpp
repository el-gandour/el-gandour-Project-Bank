...
enum enTransactions { Deposite = 1, Withdraw = 2, TotalBalances = 3, MainMeneu = 4 };

void UpdateClientBalanceInVector(string AccountNumber, vector<stBankClient>& vClient, double Amount)
{
    for (stBankClient& stClient : vClient)
    {
        if (stClient.AccountNumber == AccountNumber)
        {
            stClient.AccountBalance += Amount;
            break;
        }
    }
}

void DepositeInClientAccount(string AccountNumber, vector<stBankClient>& vClient)
{
    stBankClient stClient;
    double Deposite;
    char Answer = 'n';
    if (FindClientByAccountNumber(AccountNumber, vClient, stClient))
    {
        ...
        cout << "\nAre you sure you want to deposite to this account?(y/n): ";
        cin >> Answer;
        cin.ignore();
        if (toupper(Answer) == 'Y')
        {
            ...
            if (toupper(Answer) == 'Y')
            {
                UpdateClientBalanceInVector(AccountNumber, vClient, Deposite);
                SaveClientsDataToFile(FileName, vClient);
                cout << "\n\ntransaction done succesfully,new balance = " << stClient.AccountBalance + Deposite;
            }
        }
    }
    else
...
void WithdrawFromClientAccount(string AccountNumber, vector<stBankClient>& vClient)
{
    stBankClient stClient;
    double Withdraw;
    char Answer = 'n';
    if (FindClientByAccountNumber(AccountNumber, vClient, stClient))
    {
        ...
        if (toupper(Answer) == 'Y')
        {
            ...
            cin >> Withdraw;
            cin.ignore();
            if (Withdraw <= stClient.AccountBalance)
            {
                cout << "\nAre you sure you want to perfrom this transaction?(y/n): ";
                cin >> Answer;
                cin.ignore();
                if (toupper(Answer) == 'Y')
                {
                    UpdateClientBalanceInVector(AccountNumber, vClient, Withdraw * -1);
                    SaveClientsDataToFile(FileName, vClient);
                    cout << "\n\ntransaction done succesfully,new balance = " << stClient.AccountBalance - Withdraw;
                }
            }
            else
                cout << "the amount that you want to withdraw is more than your account balance";
        }
    }
    else
        cout << "Account number (" << AccountNumber << ") not found";
...
void ShowDepositeScreen()
{
    ...
    string AccountNumber = ReadClientAccountNumber();
    DepositeInClientAccount(AccountNumber, vClient);
}
void ShowWithdrawScreen()
{
    ...
    vector<stBankClient> vClient = LoadDataFromFileToVector(FileName);
    string AccountNumber = ReadClientAccountNumber();
    WithdrawFromClientAccount(AccountNumber, vClient);
}
void PrintClientsBalances(stBankClient stClient)
{
    cout << "| " << setw(20) << left << stClient.AccountNumber;
    cout << "| " << setw(30) << left << stClient.Name;
    cout << "| " << setw(20) << left << stClient.AccountBalance;
}
void ShowAllClientBalances()
{
    vector<stBankClient> vClient = LoadDataFromFileToVector(FileName);
    double SumBalance = 0;
    cout << "\n\t\t\t\t\tClient List (" << vClient.size() << ") Client(s).";
    ...
    for (stBankClient stClient : vClient)
    {
        PrintClientsBalances(stClient);
        SumBalance += stClient.AccountBalance;
        cout << endl;
    }
    cout << "\n_______________________________________________________";
    cout << "_________________________________________\n" << endl;
    cout << "\n\t\t\t\t\t Total Balances = " << SumBalance;
}
...
void PerfromTransactionMeneuOption(enTransactions TransactionMeneuOption)
{
    ...
    case enTransactions::TotalBalances:
        system("cls");
        ShowAllClientBalances();
        GoBackToTransactionMenue();
        break;
    case enTransactions::MainMeneu:
        break;
    }
}
...
void ShowTransactionMenue()
{
    ...
    cout << "\t[4] Main Menue.\n";
    cout << "===================================\n";
    PerfromTransactionMeneuOption((enTransactions)ReadTransactionsMenueOptions());
...
enMainMenue ReadMainMenueChoice()
{
    ...
    case 6:
        return enMainMenue::Transaction;
    case 7:
